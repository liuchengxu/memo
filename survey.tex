...
The validators on Canyon network have two duties: author blocks as a virtual miner and provide the unstoppable storage services. The security of a PoS system is rooted in the number of token in staking.
...
Specifically, the perpetual storage needs can be classified into two kinds: immortal and indefinite storage. The immortal storage means the file that is doomed to be preserved from the creation, such as the metadata of NFTs. The indefinite storage is such a kind of data that has an uncertainty of ending time, but can be deleted once it's used or the client simply does not wish the contract to be continued for any reason.

For these cases, Canyon supports the origin uploader choose to stop the contract and partially refund the charged perpetual storage fee. The files that have been refunded will be disqualified to be selected for the future storage consensus processure, thus the miners that keeps storing them will not be rewarded. For the sake of scarce storage resources, consequently the miners are expected to purge these data from the local storage, eventually they will be deleted from the network.

\subsubsection{Payments}

Canyon has an inherent advantage of easier interoperablity with the Polkadot\cite{ref4} ecosystem by using Substrate\cite{ref5}, which is the same framework Polkadot is built on. Canyon will support the users pay the storage fee by the stable coins like USDT, reducing the impact of the churn of token price on providing a stable storage service.
...
Canyon network is designed to be a permanent decentralized storage network, putting the emphsis on both lighter storage consensus and data retrieval. Being a permanent storage layer, Canyon is better suited for the storage-based computation paradigm.
